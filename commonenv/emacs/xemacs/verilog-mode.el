...
;;
;; $Id: verilog-mode.el,v 1.1.1.1 2002/08/20 19:27:00 rlford1 Exp $
...
;; AUTO features, signal, modsig; by: Wilson Snyder (wsnyder@world.std.com)
;; Keywords: languages
...
(defconst verilog-mode-version "$$Revision: 1.1.1.1 $$"
...
(defvar verilog-auto-lineup '(case) 
  ...
  (if (or verilog-tab-always-indent
      ...
      (let* (
		...
		(let (type state )
		  ...
		  (setq state (car type))
		  ;;(prompt-for-insert (format "evt: type = %s" type))
		  (cond
		   ((eq state 'block)
	  ...
	  (cond
	   ...
	   ((string-match "^[$`a-zA-Z_]" keywd)	;; not exactly word constituant
	    (cond ((or ignore-next
		       ...
		       (string-match "^`_" keywd)	;; DEG - constant
...
module that the point is over."
  (verilog-beg-of-defun)
  (let* ((end-mod-point (verilog-get-end-of-defun nil))
	 (state nil)
	 (instants-list nil))
    (save-excursion
      (while (< (point) end-mod-point)
  ...
  (let ((oldbuf (if (not (buffer-modified-p))
    ...
    (save-excursion
      ...
      (verilog-preserve-cache
       ...
       (verilog-auto-search-do "/*AS*/" 'verilog-auto-sense)
